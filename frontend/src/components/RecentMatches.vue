      ...
      >
        ...
        <div class="card-bottom">
          <div v-if="match.streams_list && match.streams_list.length > 0" class="streams-buttons">
            <button 
              v-for="(stream, index) in getVisibleStreams(match)" 
              :key="`${match.id}-stream-${index}`"
...
const sortOptions = [
  { key: 'importance-date', label: 'Campeonatos Importantes + Mais Recentes' },
...
const filteredMatches = computed(() => {
  ...
  if (sortBy.value === 'league') {
    ...
    result = [...result].sort((a, b) => 
      new Date(b.ended_at || b.scheduled_at) - new Date(a.ended_at || a.scheduled_at)
    )
  }
